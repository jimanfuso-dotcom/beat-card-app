...
def render_beat_card_image(metadata):
    ...
    using sized-up typography and generous hardware silicone pad spacing.
    """
    ...
    # --- SIZED-UP TYPOGRAPHY CALIBRATION ---
    f_top_label = get_font(20, bold=True)
    f_title = get_font(58, bold=True)
    f_sub = get_font(24, bold=False)
    f_badge = get_font(22, bold=True)
    f_inst = get_font(32, bold=True)
    f_inst_meta = get_font(21, bold=False)
    f_pad_num = get_font(28, bold=True)
    f_panel_title = get_font(30, bold=True)
    f_body_bold = get_font(24, bold=True)
    f_body = get_font(23, bold=False)
    f_tip_title = get_font(22, bold=True)
    f_tip = get_font(20, bold=False)

    # 1. TOP HEADER & BRANDING
    draw.text((62, 52), "VERTICAL BEAT BLUEPRINT  //  ABLETON PUSH 3", fill=TEXT_CYAN, font=f_top_label)

    title_text = metadata.get("title", "EXTRACTED AUDIO BEAT CARD").upper()
    draw.text((62, 88), title_text, fill=TEXT_WHITE, font=f_title)

    sub_label = f"STEP SEQUENCER HARMONIC MAP  •  {metadata.get('bar_used', 'BAR 1').upper()}"
    draw.text((62, 160), sub_label, fill=TEXT_MUTED, font=f_sub)

    # BADGES (Enlarged pills with generous padding)
    badges = [
        f"TEMPO: {metadata.get('bpm', 120)} BPM",
        f"KIT: {metadata.get('kit', 'Kit-Core 909')[:24]}",
        f"GROOVE: {metadata.get('swing', 'Straight 0%')[:26]}"
    ]
    cur_x = 62
    badge_y = 206
    badge_h = 52
    for b in badges:
        ...
        padding = 32
        badge_w = bw + padding
        draw.rounded_rectangle([cur_x, badge_y, cur_x + badge_w, badge_y + badge_h], radius=8, fill=(24, 28, 40), outline=(60, 72, 100), width=2)
        th = bbox[3] - bbox[1]
        draw.text((cur_x + 16, badge_y + (badge_h - th) // 2 - 2), b, fill=TEXT_CYAN, font=f_badge)
        cur_x += badge_w + 16

    # Divider line
    draw.line([(62, 282), (width - 62, 282)], fill=PANEL_BORDER, width=2)

    # 2. INSTRUMENT DRUM LANES (KICK, SNARE, HATS)
    tracks = [
        ("KICK DRUM", "Transients detected in sub/low-end band (< 130 Hz)", metadata.get("kick", [0]*16)),
        ("SNARE / CLAP", "Mid-frequency punch & acoustic crack backbeats (180–1400 Hz)", metadata.get("snare", [0]*16)),
        ("HI-HATS / PERCUSSION", "High-frequency top loop sizzle & 16th shuffles (> 4000 Hz)", metadata.get("hats", [0]*16))
    ]

    lane_start_y = 312
    lane_spacing = 310
    pad_w = 104
    pad_h = 88   # Sized up from 76 for bolder pad presence
    pad_gap = 18
    row_gap = 14

    for idx, (inst_name, inst_desc, steps) in enumerate(tracks):
        y_base = lane_start_y + (idx * lane_spacing)

        # Header tags
        draw.text((62, y_base), inst_name, fill=TEXT_WHITE, font=f_inst)
        draw.text((62, y_base + 40), inst_desc, fill=TEXT_MUTED, font=f_inst_meta)

        # Dual row layout: Row 1 (Steps 1-8), Row 2 (Steps 9-16)
        for step in range(16):
            row = 0 if step < 8 else 1
            col = step if step < 8 else step - 8

            px = 62 + col * (pad_w + pad_gap)
            py = y_base + 78 + row * (pad_h + row_gap)
            ...
            if val >= 80:
                ...
                border_color = (90, 140, 230)
                text_col = (210, 230, 255)
            else:
                fill_color = PAD_OFF_DOWNBEAT if is_downbeat else PAD_OFF_OFFBEAT
                border_color = PAD_OFF_BORDER
                text_col = (120, 135, 160)

            # Draw hardware silicone pad
            draw.rounded_rectangle([px, py, px + pad_w, py + pad_h], radius=10, fill=fill_color, outline=border_color, width=2)
            
            # Step number centered inside pad
            step_num = str(step + 1)
            t_bbox = draw.textbbox((0, 0), step_num, font=f_pad_num)
            tw = t_bbox[2] - t_bbox[0]
            th = t_bbox[3] - t_bbox[1]
            draw.text((px + (pad_w - tw)//2, py + (pad_h - th)//2 - 2), step_num, fill=text_col, font=f_pad_num)

    # 3. PRODUCTION & GROOVE BLUEPRINT FOOTER PANEL
    footer_y = 1250
    footer_h = 610
    draw.rounded_rectangle([62, footer_y, width - 62, footer_y + footer_h], radius=16, fill=PANEL_BG, outline=PANEL_BORDER, width=2)

    draw.text((96, footer_y + 32), "PRODUCTION & GROOVE BLUEPRINT", fill=TEXT_CYAN, font=f_panel_title)

    bullets = [
        ("Multi-Bar Spectral Quantizer: ", f"Analyzed loop and extracted {metadata.get('bar_used', 'Bar 1')} with zero-latency alignment."),
    ...
    for b_title, b_desc in bullets:
        draw.text((96, b_y), "•  " + b_title, fill=BLUE_ELECTRIC, font=f_body_bold)
        title_w = draw.textbbox((0, 0), "•  " + b_title, font=f_body_bold)[2]
        draw.text((96 + title_w + 4, b_y), b_desc, fill=TEXT_WHITE, font=f_body)
        b_y += 58

    # Hardware Tip inside box
    tip_box_y = footer_y + 350
    tip_box_h = 220
    draw.rounded_rectangle([96, tip_box_y, width - 96, tip_box_y + tip_box_h], radius=12, fill=(13, 15, 22), outline=(54, 64, 88), width=1)
    draw.text((124, tip_box_y + 26), "💡 PUSH 3 HARDWARE PROGRAMMING TIP", fill=AMBER, font=f_tip_title)

    tip_line1 = "Press Push 3's 'Note' button to enter 16-step sequencer mode. Select Drum Rack pad"
    tip_line2 = "(Kick/Snare/Hat) and tap the lit pads above. Turn the Swing encoder to match detected groove."
    draw.text((124, tip_box_y + 74), tip_line1, fill=TEXT_MUTED, font=f_tip)
    draw.text((124, tip_box_y + 110), tip_line2, fill=TEXT_MUTED, font=f_tip)
